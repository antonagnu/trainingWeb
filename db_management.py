...
def getBestWeek():
    try:
        ...
        cur.execute('SELECT strftime(\"%Y-%W\", Date ) as \"Week\", sum(Distance) as Dist FROM activitiesTb GROUP BY strftime(\"%Y-%W\", Date) ORDER BY Dist DESC LIMIT 1;')
        result=cur.fetchone()
        conn.close()

        d= str(result[0])
        d1=d[0:5]+"W"+d[5:]
        week = datetime.datetime.strptime(d1 + '-1', "%Y-W%W-%w")
        weekOk=str(week)
        weekOk = weekOk[0:10]
        distance = result[1]

        return distance, weekOk
...
def getLocations():
    try:
        conn=sqlite3.connect("runningLog.sqlite")
        cur=conn.cursor()
        cur.execute('SELECT Title FROM activitiesTb GROUP BY Title')
        result=cur.fetchall()
        conn.close()
        return result
